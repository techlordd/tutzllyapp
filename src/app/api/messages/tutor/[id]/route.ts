...
export async function PUT(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await queryOne(
    `UPDATE messages_tutor SET status=$1, last_updated=NOW() WHERE record_id=$2 RETURNING *`,
    [status, Number(id)]
  );
...
export async function DELETE(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  const { id } = await params;
  await queryOne(
    `UPDATE messages_tutor SET entry_status='deleted', last_updated=NOW() WHERE record_id=$1`,
    [Number(id)]
  );
