  ...
  try {
    ...
    const schedule = await queryOne(
      `UPDATE schedules SET day=$1, session_start_time=$2, session_end_time=$3, duration=$4,
       time_zone=$5, zoom_link=$6, meeting_id=$7, meeting_passcode=$8, assign_status=$9, last_updated=NOW()
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const { id } = await params;
    await query(`UPDATE schedules SET entry_status='deleted', last_updated=NOW() WHERE schedule_id=$1`, [id]);
