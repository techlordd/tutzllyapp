...
interface Student {
  ...
  fathers_name: string; fathers_email: string; created_at: string;
}
...
export default function StudentsPage() {
  ...
  const columns = [
    { key: 'fullname', label: 'Student', render: (_: unknown, row: Student) => (
    ...
    <DashboardLayout title="Students Management">
      <div className="space-y-5">
        ...
        <DataTable data={students} columns={columns} loading={loading}
          ...
          actions={(row) => (
            <>
              <button onClick={() => { setSelectedStudent(row); setViewModalOpen(true); }}
      ...
      <Modal isOpen={modalOpen} onClose={() => { setModalOpen(false); resetForm(); }}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <FormField label="Grade"><Select value={form.grade} onChange={e => setForm({...form, grade: e.target.value})}><option value="">Select Grade</option>{GRADES.map(g => <option key={g}>{g}</option>)}</Select></FormField>
            <FormField label="School"><Input value={form.school} onChange={e => setForm({...form, school: e.target.value})} /></FormField>
            ...
            <FormField label="Status"><Select value={form.status} onChange={e => setForm({...form, status: e.target.value})}><option value="active">Active</option><option value="inactive">Inactive</option></Select></FormField>
          </div>
          <div className="border-t pt-4">
            <h4 className="font-semibold text-sm text-gray-700 mb-3">Parent/Guardian Info</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <FormField label="Mother's Name"><Input value={form.mothers_name} onChange={e => setForm({...form, mothers_name: e.target.value})} /></FormField>
              <FormField label="Mother's Email"><Input type="email" value={form.mothers_email} onChange={e => setForm({...form, mothers_email: e.target.value})} /></FormField>
              <FormField label="Father's Name"><Input value={form.fathers_name} onChange={e => setForm({...form, fathers_name: e.target.value})} /></FormField>
              <FormField label="Father's Email"><Input type="email" value={form.fathers_email} onChange={e => setForm({...form, fathers_email: e.target.value})} /></FormField>
            </div>
          </div>
          <FormField label="Short Bio"><Textarea rows={3} value={form.short_bio} onChange={e => setForm({...form, short_bio: e.target.value})} /></FormField>
      ...
      {selectedStudent && (
        <Modal isOpen={viewModalOpen} onClose={() => setViewModalOpen(false)} title="Student Profile" size="lg">
          <div className="space-y-4">
            <div className="flex items-center gap-4 pb-4 border-b">
              <Avatar name={`${selectedStudent.firstname} ${selectedStudent.surname}`} size="xl" />
              <div>
                <h3 className="text-xl font-bold">{selectedStudent.firstname} {selectedStudent.surname}</h3>
