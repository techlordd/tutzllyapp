...
export default function AdminMissedSessionsPage() {
  ...
  const fetchSessions = useCallback(async () => {
    setLoading(true);
    try {
      const res = await fetch('/api/sessions?status=missed');
  ...
  const columns = [
    { key: 'tutor_firstname', label: 'Tutor', render: (_: unknown, row: MissedSession) =>
      ...
      {modal && (
        <Modal isOpen={true} onClose={() => setModal(null)} title="Reschedule Session" size="sm">
          <div className="bg-gray-50 rounded-xl p-3 mb-4 text-sm space-y-0.5">
            <p><span className="font-medium">Tutor:</span> {`${modal.tutor_firstname || ''} ${modal.tutor_lastname || ''}`.trim() || '—'}</p>
