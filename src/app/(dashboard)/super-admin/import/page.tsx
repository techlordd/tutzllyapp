...
export default function SuperAdminImportPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult(data);
      if (data.inserted > 0) toast.success(`${data.inserted} record${data.inserted !== 1 ? 's' : ''} imported`);
      else if (data.errors.length === 0) toast.success('Import complete — all records already existed');
